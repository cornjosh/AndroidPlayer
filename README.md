...
- [Day14](#day14)
  ...
  - [已实现功能](#已实现功能)
    - [附加功能](#附加功能)
  - [主要难点](#主要难点)
    - [1. **AAudio 播放器无法播放音频**](#1-aaudio-播放器无法播放音频)
    - [2. **音频无法正确播放和流的关闭**](#2-音频无法正确播放和流的关闭)
    - [3. **获取和同步音频的 PTS**](#3-获取和同步音频的-pts)
    - [4. **音视频同步问题**](#4-音视频同步问题)
    - [5. **线程安全的时间戳变量**](#5-线程安全的时间戳变量)
    - [6. **音频和视频线程的时间戳同步**](#6-音频和视频线程的时间戳同步)
    - [7. **编译成功但播放音视频依旧不同步**](#7-编译成功但播放音视频依旧不同步)
    - [8. **解码线程（`decodeThread`）退出后播放中断**](#8-解码线程decodethread退出后播放中断)
    - [9. **解码帧的内存泄漏问题**](#9-解码帧的内存泄漏问题)
    - [10. **Surface 未正确显示导致黑屏**](#10-surface-未正确显示导致黑屏)
    - [11. **EGL 初始化重复问题**](#11-egl-初始化重复问题)
    - [12. **重复解码导致的性能问题**](#12-重复解码导致的性能问题)
    - [13. **解码帧的 `linesize` 为 0**](#13-解码帧的-linesize-为-0)
    - [总结：](#总结)
  - [以实现视频和音频的同步播放的目标](#以实现视频和音频的同步播放的目标)
- [Day13](#day13)
  - [作业要求](#作业要求-1)
  - [作业完成录屏](#作业完成录屏)
- [Day12](#day12)
