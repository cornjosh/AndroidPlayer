...
在实现视频播放时，我遇到了一个非常关键的问题：**解码线程（`decodeThread`）执行完后就终止，导致视频播放中断。** 我原本的代码在解码线程完成后便退出了，导致播放器没有继续处理后续的视频帧。

**解决方案**：
- 在调试过程中，我发现 `decodeThread` 线程的退出条件设置不够合理，导致它在处理完所有的包后就直接退出了。为了让解码线程继续工作，我修改了退出条件，确保解码线程只有在队列完全消费完且没有更多数据时才退出。
- 我增加了一个机制，当 `packetQueue` 中没有更多数据时，解码线程会稍作等待，直到有新数据进来，避免线程提前退出。

---

### 9. **解码帧的内存泄漏问题**

播放过程中，设备死机的问题在很大程度上是由于内存泄漏引起的，特别是在解码阶段，我没有及时释放已解码的 `AVFrame`。这导致内存不断积累，最终设备内存溢出，系统被强制杀掉。

**解决方案**：
- 我加强了内存管理，在每次解码并将帧推入队列后，确保及时释放不再使用的帧。特别是在使用完 `AVFrame` 后，我通过 `av_frame_free()` 来确保内存得到正确释放。
- 另外，为了避免每次都创建和销毁大量 `AVFrame` 对象，我实现了帧的复用机制，避免频繁的内存分配和释放。

---

### 10. **Surface 未正确显示导致黑屏**

在我将渲染帧通过 OpenGL 渲染到 `SurfaceView` 时，发现渲染输出黑屏。经过调试，我发现是因为 `SurfaceView` 没有正确设置约束，导致渲染的 Surface 尺寸为 0x0。

**解决方案**：
- 我确保在 `SurfaceView` 使用 `ConstraintLayout` 时，为其添加了正确的约束（`layout_constraintTop_toTopOf`、`layout_constraintStart_toStartOf` 等），这样 `SurfaceView` 能够正确渲染到屏幕上。
- 同时，我还在 `SurfaceView` 中设置了调试背景色，以确保视图显示正常，并且进一步验证了 `Surface` 在渲染时的宽高正确。

---

### 11. **EGL 初始化重复问题**

当我实现了渲染后，发现在播放过程中，`EGL` 被重复初始化，导致了渲染输出异常或黑屏。重复初始化 EGL 会破坏现有的上下文，导致图形渲染失败。

**解决方案**：
- 我使用了 `std::mutex` 来加锁确保每次只初始化一次 `EGL` 上下文。每次渲染前，我检查 `EGL` 是否已经初始化，并且只有在必要时才重新初始化。
- 此外，我还增加了对 `Surface` 和 `EGLContext` 的管理，确保在 `Surface` 或视图尺寸变化时，能够安全地重建 `EGL` 上下文。

---

### 12. **播放过程中线程卡死或应用崩溃**

播放过程中，我遇到了设备死机的情况，这往往是由于线程没有正确退出、解码线程与渲染线程之间的死锁或资源竞争导致的。尤其是，`decodeThread` 和 `renderThread` 在某些情况下会相互阻塞。

**解决方案**：
- 我通过加入 `std::atomic` 和 `std::mutex` 解决了线程间的同步问题，确保解码线程和渲染线程不会在资源竞争时发生阻塞。
- 我使用了条件变量来确保解码线程和渲染线程在合适的时机进行同步，避免了死锁现象。
- 同时，我还确保了在 `onPause` 或 `onStop` 时，能够安全地停止所有解码和渲染线程，避免了线程阻塞导致的崩溃。

---

### 13. **重复解码导致的性能问题**

随着播放时间的增加，我还遇到了解码性能逐渐下降的问题，尤其是在帧队列中积压了大量待处理的解码帧，导致解码线程长时间没有释放 CPU 资源，从而影响了播放流畅性。

**解决方案**：
- 为了避免解码线程被帧队列中的数据积压，我设计了一个最大缓存机制，限制帧队列中最多缓存的帧数，从而避免解码线程长时间被阻塞。
- 另外，我还通过减少渲染线程的渲染频率（比如调整帧率）来减轻 GPU 和 CPU 的负担，从而提高整体性能。

---

### 14. **解码帧的 `linesize` 为 0**

在播放过程中，我遇到了一个问题，解码帧的 `linesize[0]` 值为 0，导致图像无法正确渲染。经过调试，我发现是因为在解码和转换为 RGBA 格式的过程中，未正确处理帧的数据格式或内存分配问题。

**解决方案**：
- 我确保每次解码后都为 RGBA 帧分配新的内存，并使用 `sws_scale()` 来转换为 RGBA 格式。
- 同时，我增加了 `linesize` 的有效性检查，确保每帧的 `linesize` 不为零，避免了空帧的渲染。
